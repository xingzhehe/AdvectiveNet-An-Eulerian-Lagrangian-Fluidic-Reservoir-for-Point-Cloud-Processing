...
from classification10.modelnet10 import *
import time
import matplotlib.pyplot as plt
from mpl_toolkits.mplot3d import Axes3D
from src.point_cloud_data_augmentation import *
from classification10.advection_net import AdvectionNet
...
if __name__ == '__main__':
    ...
    data_rand = True
    data_aug = True
    dataset = ModelNet10Dataset(data_type, num_points=1024)
    shape = "bed"
    index = find_pc(dataset, shape, 2)
    ...
    net = AdvectionNet(num_class=10, grid_size=8, sigma=0.1, time_step=3, pic_flip=0.3, batch_norm=True)
    net.load_state_dict(torch.load("../modelnet10_log/modelnet101.pt", map_location=lambda storage, location: storage))
    torch.set_grad_enabled(False)
    net.eval()
    data = torch.from_numpy(data).unsqueeze(0)
    scores = net.forward_with_view(data)
