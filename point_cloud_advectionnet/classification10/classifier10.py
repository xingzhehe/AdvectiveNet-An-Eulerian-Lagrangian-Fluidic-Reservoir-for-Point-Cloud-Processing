...
from classification10.modelnet10 import ModelNet10Dataset
...
class Classifier10:
    ...
    def train(self):
        ...
        log_file.close()
        train_data = torch.utils.data.DataLoader(ModelNet10Dataset("train", num_points=self.hp.num_points,),
                                                 batch_size=self.hp.batch_size,
                                                 shuffle=True,
                                                 num_workers=self.hp.workers)
        test_data = torch.utils.data.DataLoader(ModelNet10Dataset("test", num_points=self.hp.num_points),
                                                batch_size=self.hp.batch_size,
                                                num_workers=self.hp.workers)
        ...
        for epoch in range(self.hp.max_epoch):
            ...
            for batch_index, data_batch in enumerate(train_data):
                ...
                if self.hp.label_smooth:
                    loss = label_smooth_loss(scores, label.to(self.hp.device), confidence=0.8) + self.hp.penalty * penalty
                else:
